...
The work-order deployer must not replace this template with its generic container defaults. Before applying a new image, render the release template with `node scripts/deployment-contract.mjs <full-image-reference> 8080`. The command rejects an unsafe replica count, missing durable environment value, missing mount, or wrong storage binding, then replaces only the example image tag and `PORT`. `npm run test:deployment-contract` covers the exact unsafe topology observed in verification 11.
