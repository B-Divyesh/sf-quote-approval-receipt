...
`DATA_DIR` remains the image default (`/data`) so SQLite uses local fast storage and copies each committed snapshot to the mounted share. Do not scale this SQLite deployment above one replica: SQLite has one writer and Azure Files is used for durable replacement snapshots, not concurrent database locking. The durable-snapshot claim test commits 20 demos, replaces the process with a new local data directory, and reads all 20 through the new process. Live verification must create and read at least 20 fresh demo workspaces across new connections.
